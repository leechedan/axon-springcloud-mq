...
public enum CacheUtils {
    ;
    /**
     * cache的存储目录. 若使用docker, 考虑是否需要做挂载配置
     */
    private static final String CACHE_STORAGE_PATH = "/tmp/cache";

    /**
     * 缓存可用的堆内存size, 以 对象 为单位 谨慎使用，影响gc效率
     */
    ...
    public static Cache<String, AggregateCacheEntry> createCacheInstance(String cacheName) {
        ...
        CacheConfiguration<String, AggregateCacheEntry> cacheConfiguration = CacheConfigurationBuilder
                ...
                // 超时配置
                .withExpiry(Expirations.timeToLiveExpiration(Duration.of(CACHE_EXPIRATION_SECONDS, TimeUnit.SECONDS)))
                .build();
        // JCache 缓存对象
     ...
     * @return
     */
    public static Cache<String, AggregateCacheEntry> createCacheInstance(String cacheName, long headSize,
                                                                         long offHeadSize, long diskSize,
                                                                         long expirationTime) {
