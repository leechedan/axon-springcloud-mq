...
public class StringUtil {
    ...
    public static String getPathStr(String para) {
        return para.replace(UNDERLINE, "-");
    }

    public static String addCharEnd(String para, String endChar){

        if (StringUtils.isBlank(para)){
            return "";
        }
        return !para.endsWith(endChar)? para + endChar : para;
    }

    public static String upperFstChar(String para) {
        StringBuilder result = new StringBuilder();
        String a[] = para.split(UNDERLINE);
        for (String s : a) {
            result.append(s.substring(0, 1).toUpperCase());
